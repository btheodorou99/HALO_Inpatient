...
from config import HALOCoarseConfig

SEED = 4
random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)
config = HALOCoarseConfig()
...
model = HALOCoarseModel(config).to(device)
...
pickle.dump(intermediate, open("../../results/testing_stats/HALO_Coarse_intermediate_results.pkl", "wb"))
...
pickle.dump(metrics_dict, open("../../results/testing_stats/HALO_Coarse_Metrics.pkl", "wb"))
...
synthetic_ehrs = sample_sequence(model, config.n_ctx, stoken, batch_size=3, device=device, sample=True)
...
pickle.dump(synthetic_ehr_dataset, open(f'../../results/datasets/haloCoarseDataset.pkl', 'wb'))