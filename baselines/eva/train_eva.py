...
for e in tqdm(range(config.epoch)):
  ...
  for i in range(0, len(train_ehr_dataset), config.batch_size):
    ...
    batch_ehr, batch_lens, batch_mask = get_batch(i, config.batch_size, 'train')
    batch_ehr = torch.tensor(batch_ehr, dtype=torch.float32).to(device)
    batch_mask = torch.tensor(batch_mask, dtype=torch.float32).to(device)

    optimizer.zero_grad()
    loss, _, _ = model(batch_ehr, batch_lens, ehr_labels=batch_ehr, ehr_masks=batch_mask, pos_loss_weight=config.pos_loss_weight, kl_weight=klw)
    loss.backward()
    optimizer.step()
    
    if i % (100*config.batch_size) == 0:
      print("Epoch %d, Iter %d: Training Loss:%.6f"%(e, i, loss))
    if i % (200*config.batch_size) == 0:
      if i == 0:
        continue
      ...
      with torch.no_grad():
        ...
        for v_i in range(0, len(val_ehr_dataset), config.batch_size):
          batch_ehr, batch_lens, batch_mask = get_batch(v_i, config.batch_size, 'valid')
          batch_ehr = torch.tensor(batch_ehr, dtype=torch.float32).to(device)
          batch_mask = torch.tensor(batch_mask, dtype=torch.float32).to(device)

          val_loss, _, _ = model(batch_ehr, batch_lens, ehr_labels=batch_ehr, ehr_masks=batch_mask, pos_loss_weight=config.pos_loss_weight, kl_weight=klw)
        ...
        if cur_val_loss < global_loss:
          ...
          torch.save(state, '../../save/eva_model')
          print('\n------------ Save best model ------------\n')