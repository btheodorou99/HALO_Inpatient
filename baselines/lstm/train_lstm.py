...
if os.path.exists("../../save/lstm_model"):
...
for e in tqdm(range(config.epoch)):
  ...
  for i in range(0, len(train_ehr_dataset), config.batch_size):
    ...
    optimizer.zero_grad()
    loss, _, _ = model(batch_ehr, ehr_labels=batch_ehr, ehr_masks=batch_mask, pos_loss_weight=config.pos_loss_weight)
    loss.backward()
    optimizer.step()
    ...
    if i % (100*config.batch_size) == 0:
      ...
      with torch.no_grad():
        ...
        for v_i in range(0, len(val_ehr_dataset), config.batch_size):
          ...
          val_loss, _, _ = model(batch_ehr, ehr_labels=batch_ehr, ehr_masks=batch_mask, pos_loss_weight=config.pos_loss_weight)
        ...
        if cur_val_loss < global_loss:
          ...
          torch.save(state, '../../save/lstm_model')
          print('\n------------ Save best model ------------\n')