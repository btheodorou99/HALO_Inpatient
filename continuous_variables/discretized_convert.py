...
for p in dataset:
  ...
  for v in p['visits']:
    ...
    for i in range(len(v[2])):
      new_labs.append(idToLab[v[2][i]])
      if isCategorical[idToLab[v[2][i]]]:
        new_values.append(possibleValues[idToLab[v[2][i]]][v[3][i]])
      else:
        new_values.append(formatCont(random.uniform(discretization[idToLab[v[2][i]]][v[3][i]], discretization[idToLab[v[2][i]]][v[3][i]+1]), idToLab[v[2][i]]))
    ...
    new_visits.append((v[1], new_labs, new_values, [new_cont]))
  p['visits'] = new_visits
